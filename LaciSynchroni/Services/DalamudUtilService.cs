...
public class DalamudUtilService : IHostedService, IMediatorSubscriber
{
    ...
    public DalamudUtilService(ILogger<DalamudUtilService> logger, IDalamudPluginInterface pluginInterface, IClientState clientState, IObjectTable objectTable, IFramework framework,
        IGameGui gameGui, ICondition condition, IDataManager gameData, ITargetManager targetManager, IGameConfig gameConfig,
        BlockedCharacterHandler blockedCharacterHandler, SyncMediator mediator, PerformanceCollectorService performanceCollector,
        SyncConfigService configService)
    {
        ...
        WorldData = new(() =>
        {
            return gameData.GetExcelSheet<World>(ClientLanguage.English)!
                .Where(w => !w.Name.IsEmpty && w.DataCenter.RowId != 0 && (w.IsPublic || char.IsUpper(w.Name.ToString()[0])))
                .ToDictionary(w => (ushort)w.RowId, w => w.Name.ToString());
        });
        JobData = new(() =>
        {
            return gameData.GetExcelSheet<ClassJob>(ClientLanguage.English)!
                .ToDictionary(k => k.RowId, k => k.NameEnglish.ToString());
        });
        TerritoryData = new(() =>
        {
            return gameData.GetExcelSheet<TerritoryType>(ClientLanguage.English)!
            .Where(w => w.RowId != 0)
        ...
        MapData = new(() =>
        {
            return gameData.GetExcelSheet<Map>(ClientLanguage.English)!
            .Where(w => w.RowId != 0)
            .ToDictionary(w => w.RowId, w =>
            {
                StringBuilder sb = new();
    ...
    private unsafe void FrameworkOnUpdateInternal()
    {
        ...
        _performanceCollector.LogPerformance(this, $"FrameworkOnUpdateInternal+{(isNormalFrameworkUpdate ? "Regular" : "Delayed")}", () =>
        {
            ...
            if (_gameConfig != null
                && _gameConfig.TryGet(SystemConfigOption.LodType_DX11, out bool lodEnabled))
            {
