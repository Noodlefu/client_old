    ...
    IChatGui chatGui, SyncConfigService configurationService) : DisposableMediatorSubscriberBase(logger, mediator), IHostedService
{
    ...
    {
        var se = new SeStringBuilder().AddText($"[{_dalamudUtilService.GetPluginName()}] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        var se = new SeStringBuilder().AddText($"[{_dalamudUtilService.GetPluginName()}] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        var se = new SeStringBuilder().AddText($"[{_dalamudUtilService.GetPluginName()}] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
