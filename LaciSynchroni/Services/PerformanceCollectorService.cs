...
public sealed class PerformanceCollectorService(ILogger<PerformanceCollectorService> logger, SyncConfigService syncConfigService) : IHostedService
{
    ...
    {
        if (!_syncConfigService.Current.LogPerformance) return func.Invoke();
    ...
    public void LogPerformance(object sender, CustomInterpolatedStringHandler counterName, Action act, int maxEntries = 10000)
    {
        if (!_syncConfigService.Current.LogPerformance) { act.Invoke(); return; }
