...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    public Task<(CharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = CharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read Laci Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(CharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        CharaFileHeader.AdvanceReaderToData(reader);
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            var charaFileData = _charaFileDataFactory.Create(description, data);
            CharaFileHeader output = new(CharaFileHeader.CurrentVersion, charaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving MCD File, deleting output");
