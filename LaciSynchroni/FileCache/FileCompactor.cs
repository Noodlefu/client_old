...
public sealed class FileCompactor
{
    ...
    public FileCompactor(ILogger<FileCompactor> logger, SyncConfigService syncConfigService, DalamudUtilService dalamudUtilService)
    {
    ...
    public void CompactStorage(bool compress)
    {
        ...
        int currentFile = 1;
        var allFiles = Directory.EnumerateFiles(_syncConfigService.Current.CacheFolder).ToList();
    ...
    public async Task WriteAllBytesAsync(string filePath, byte[] decompressedFile, CancellationToken token)
    {
        ...
        if (_dalamudUtilService.IsWine || !_syncConfigService.Current.UseCompactor)
        {
            return;
        }
