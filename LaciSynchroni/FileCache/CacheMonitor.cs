...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
        ...
        FileCompactor fileCompactor, BackgroundTaskTracker taskTracker) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<PenumbraInitializedMessage>(this, (_) =>
        {
            ...
            StartFileWatcher(configService.Current.CacheFolder);
            InvokeScan();
        });
    ...
    {
        HaltScanLocks.TryAdd(source, 0);
        HaltScanLocks[source]++;
    }

    private sealed record WatcherChange(WatcherChangeTypes ChangeType, string? OldPath = null);
    private readonly Dictionary<string, WatcherChange> _watcherChanges = new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, WatcherChange> _syncChanges = new(StringComparer.OrdinalIgnoreCase);

    public void StopMonitoring()
    {
        Logger.LogInformation("Stopping monitoring of Penumbra and Laci storage folders");
        SyncWatcher?.Dispose();
    ...
    public void StartFileWatcher(string? syncedFilesDir)
    {
        SyncWatcher?.Dispose();
        if (string.IsNullOrEmpty(syncedFilesDir) || !Directory.Exists(syncedFilesDir))
        {
            SyncWatcher = null;
            Logger.LogWarning("Laci file path is not set, cannot start the FSW for Laci.");
            return;
        }
        ...
        Logger.LogInformation("Laci Storage is on NTFS drive: {IsNtfs}", StorageIsNtfs);

        Logger.LogDebug("Initializing Laci FSW on {Path}", syncedFilesDir);
        SyncWatcher = new FileSystemWatcher
        {
            Path = syncedFilesDir,
        ...
        SyncWatcher.Deleted += SyncWatcherFileChanged;
        SyncWatcher.Created += SyncWatcherFileChanged;
        SyncWatcher.EnableRaisingEvents = true;
    }

    private void SyncWatcherFileChanged(object sender, FileSystemEventArgs e)
    {
        Logger.LogTrace("Sync FSW: FileChanged: {Change} => {Path}", e.ChangeType, e.FullPath);
    ...
    private CancellationTokenSource _syncFswCts = new();
    public FileSystemWatcher? PenumbraWatcher { get; private set; }
    public FileSystemWatcher? SyncWatcher { get; private set; }

    private async Task SyncWatcherExecution()
    {
        _syncFswCts = _syncFswCts.CancelRecreate();
        var token = _syncFswCts.Token;
        var delay = TimeSpan.FromSeconds(5);
        Dictionary<string, WatcherChange> changes;
        lock (_syncChanges)
            changes = _syncChanges.ToDictionary(t => t.Key, t => t.Value, StringComparer.Ordinal);
        try
        {
            do
            {
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(_configService.Current.CacheFolder) || !Directory.Exists(_configService.Current.CacheFolder))
        {
            ...
            Logger.LogWarning("Laci cache directory is not set or does not exist.");
        }
