...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    private const string _notesEnd = "##LACI_SYNCHRONI_USER_NOTES_END##";
    private const string _notesStart = "##LACI_SYNCHRONI_USER_NOTES_START##";
    ...
    public void DrawCacheDirectorySetting()
    {
        ...
        ImGui.SameLine();
        using (ImRaii.Disabled(_cacheMonitor.SyncWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog($"Pick {_dalamudUtil.GetPluginName()} Storage Folder", (success, path) =>
                {
                    if (!success) return;
                        ...
                        && _cacheDirectoryIsValidPath)
                    {
                        ...
                        _configService.Save();
                        _cacheMonitor.StartFileWatcher(path);
                        _cacheMonitor.InvokeScan();
                    }
                }, _dalamudUtil.IsWine ? @"Z:\" : @"C:\");
            }
        }
        if (_cacheMonitor.SyncWatcher != null)
        {
        ...
        {
            ColorTextWrapped($"Your selected directory has files or directories inside that are not from {_dalamudUtil.GetPluginName()}. Use an empty directory or a previous {_dalamudUtil.GetPluginName()} storage directory only.", ImGuiColors.DalamudRed);
        }
        else if (!_cacheDirectoryIsValidPath)
        {
            ColorTextWrapped("Your selected directory contains illegal characters unreadable by FFXIV. " +
                             "Restrict yourself to latin letters (A-Z), underscores (_), dashes (-) and arabic numerals (0-9).", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText($"The storage is automatically governed by {_dalamudUtil.GetPluginName()}. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
    ...
    public bool DrawOtherPluginState()
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, $"You need to install both Penumbra and Glamourer and keep them up-to-date to use {_dalamudUtil.GetPluginName()}.");
            return false;
        }
