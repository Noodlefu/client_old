...
public class DownloadUi : WindowMediatorSubscriberBase
{
    ...
    public DownloadUi(ILogger<DownloadUi> logger, DalamudUtilService dalamudUtilService, SyncConfigService configService,
        ...
        : base(logger, mediator, dalamudUtilService.GetPluginName() + " Downloads", performanceCollectorService)
    {
    ...
    protected override void DrawInternal()
    {
        if (_configService.Current.ShowTransferWindow)
        {
            try
            {
                if (_fileTransferManager.GetCurrentUploads().Count != 0)
                {
                    ...
                    if (!_currentDownloads.IsEmpty) ImGui.Separator();
                }
            }
            catch
            {
    ...
    public override bool DrawConditions()
    {
        ...
        if (_currentDownloads.IsEmpty && _fileTransferManager.GetCurrentUploads().Count == 0 && _uploadingPlayers.IsEmpty) return false;
