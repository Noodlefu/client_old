...
public class PopoutProfileUi : WindowMediatorSubscriberBase
{
    ...
    public PopoutProfileUi(ILogger<PopoutProfileUi> logger, SyncMediator mediator, UiSharedService uiBuilder,
        ServerConfigurationManager serverManager, SyncConfigService syncConfigService,
        ProfileManager profileManager, PairManager pairManager, PerformanceCollectorService performanceCollectorService) : base(logger, mediator, "###LaciSynchroniPopoutProfileUI", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<CompactUiChange>(this, (msg) =>
        {
            ...
            if (syncConfigService.Current.ProfilePopoutRight)
            {
    ...
    protected override void DrawInternal()
    {
        ...
        try
        {
            ...
            var descText = syncProfile.Description;
            ...
            UiSharedService.TextWrapped(trimmed ? descText + $"...{Environment.NewLine}[Open Full Profile for complete description]" : syncProfile.Description);
            font.Dispose();
