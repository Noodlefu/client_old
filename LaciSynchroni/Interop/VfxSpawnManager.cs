...
public unsafe class VfxSpawnManager : DisposableMediatorSubscriberBase
{
    ...
    public VfxSpawnManager(ILogger<VfxSpawnManager> logger, IGameInteropProvider gameInteropProvider, SyncMediator syncMediator)
        : base(logger, syncMediator)
    {
        gameInteropProvider.InitializeFromAttributes(this);
        syncMediator.Subscribe<GposeStartMessage>(this, (msg) =>
        {
            ChangeSpawnVisibility(0f);
        });
        syncMediator.Subscribe<GposeEndMessage>(this, (msg) =>
        {
            RestoreSpawnVisiblity();
        });
        syncMediator.Subscribe<CutsceneStartMessage>(this, (msg) =>
        {
            ChangeSpawnVisibility(0f);
        });
        syncMediator.Subscribe<CutsceneEndMessage>(this, (msg) =>
        {
            RestoreSpawnVisiblity();
        });
    }
